...
class BybitTradeClient(core.BybitPrivateClient):
    ...
    def set_leverage_one(self, symbol):
        return self._post(
...
class LiveApproval:
    ...
    def open(self, symbol, duration, mode="test"):
        with self.lock:
            ...
            self.client.set_leverage_one(symbol)
...
<p><button {disabled} type='submit'>Approve & Open ≈$5 Test</button></p>
</form>
  ...
  setInterval(tick,1000);
        ...
        def do_GET(self):
            ...
            if path in ("/", "/health"):
                ...
                self.send_json(
                    {"version": "0.3", "signals": latest_signals(db)}
                )
            elif path == "/trade":
...
async def run():
    ...
    log.info("trade-engine v0.3")
    log.info("AUTO_LIVE_ORDERS_ENABLED=False")
    log.info(
        "MANUAL_LIVE_APPROVAL_ENABLED=%s approval_pin_configured=%s max_notional=%s leverage=1x",
        MANUAL_LIVE_APPROVAL_ENABLED,
        bool(APPROVAL_PIN),
        MAX_LIVE_NOTIONAL_USDT,
    )
