...
class BybitTradeClient(core.BybitPrivateClient):
    def _post(self, path, body, allow_codes=None):
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "trade-engine-v0.3.2-demo",
            },
            method="POST",
        )
        try:
    ...
    def public_get(self, path, params=None):
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "trade-engine-v0.3.2-demo"})
        try:
            with urllib.request.urlopen(req, timeout=12) as response:
                data = json.loads(response.read().decode())
        except urllib.error.HTTPError as exc:
        ...
        with self.lock:
            ...
            for p in positions:
                ...
                if str(p.get("position_idx")) not in {"0", "None"}:
                    raise RuntimeError(
                        "Hedge mode is not supported in V0.3.2"
                    )
            ...
            if positions:
                pos = "<br>".join(
                    (
                        ...
                        f"lev={html.escape(str(p['leverage']))}x "
...
<body>
<h1>Trade Engine V0.3.2 Demo</h1>
<p><b>Manual approval only.</b> Auto-live is hard-disabled. Per OPEN: ${float(MAX_LIVE_NOTIONAL_USDT):,.2f} notional, {DEMO_LEVERAGE}x, max {MAX_OPEN_LINEAR_POSITIONS} open linear positions.</p>
{msg}
...
<small>Each successful OPEN becomes the new global countdown start. Same-symbol one-way orders may merge on Bybit.</small>
</div>
...
<button {disabled} type='submit'>Approve & Close All Open Positions (reduce-only)</button>
</form>
<small>Never automatic. Backend re-reads and validates all open positions before reduce-only close.</small>
</div>
  ...
  setInterval(tick,1000);
        ...
        def do_GET(self):
            ...
            if path in ("/", "/health"):
                ...
                self.send_json(
                    {"version": "0.3.2-demo", "signals": latest_signals(db)}
                )
            elif path == "/trade":
...
async def run():
    ...
    if DEMO_LEVERAGE != 20:
        raise RuntimeError("V0.3.2 demo guard: DEMO_LEVERAGE must be 20")
    if MAX_LIVE_NOTIONAL_USDT != Decimal("50000"):
        raise RuntimeError("V0.3.2 demo guard: MAX_LIVE_NOTIONAL_USDT must be 50000")
    if MAX_OPEN_LINEAR_POSITIONS != 10:
        raise RuntimeError("V0.3.2 demo guard: MAX_OPEN_LINEAR_POSITIONS must be 10")
    ...
    log.info("trade-engine v0.3.2-demo")
