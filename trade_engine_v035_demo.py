...
class BybitTradeClient(core.BybitPrivateClient):
    def _post(self, path, body, allow_codes=None):
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "trade-engine-v0.3.5-demo",
            },
            method="POST",
        )
        try:
    ...
    def public_get(self, path, params=None):
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "trade-engine-v0.3.5-demo"})
        try:
            with urllib.request.urlopen(req, timeout=12) as response:
                data = json.loads(response.read().decode())
        except urllib.error.HTTPError as exc:
...
class LiveApproval:
    ...
    def open(self, symbol, duration, mode="test", expected_ts=None, expected_direction=None):
        with self.lock:
            if not self.ready:
            ...
            # Snapshot lock: execute only the exact signal card the user approved.
            ...
            if expected_direction:
                ...
                if direction != expected_direction:
                    raise RuntimeError(
                        f"Signal direction changed from {expected_direction} to {direction}. No order sent; tap again."
                    )
            ...
            log.warning(
                "MANUAL DEMO OPEN approved symbol=%s side=%s score=%s signal_ts=%s qty=%s notional≈%.4f duration=%ss",
                symbol,
                direction,
                sig["score"],
                sig["ts_ms"],
    ...
    def close(self):
        with self.lock:
            ...
            for p in positions:
                ...
                if str(p.get("position_idx")) not in {"0", "None"}:
                    raise RuntimeError(
                        "Hedge mode is not supported in V0.3.5"
                    )
...
.controls{{display:flex;gap:7px;flex-wrap:wrap;margin-bottom:10px;position:sticky;top:0;z-index:8;background:rgba(11,14,19,.94);backdrop-filter:blur(10px);padding:8px 0}}
...
<div class='shell'>
  <div class='topbar'>
    <div>
      ...
      <h1>Trade Engine V0.3.5</h1>
      <p class='subtitle'>30-coin research · 60s engine · 3s screen sync</p>
    </div>
...
function render(payload){{
  ...
  const openSet=new Set(payload.open_symbols||INITIAL.open_symbols||[]);
  let filtered=rows.filter(r=>{{
    if(activeFilter==='ALL') return true;
    if(activeFilter==='WARMING') return r.score===null;
    if(r.score===null) return false;
    const s=Number(r.score);
    if(activeFilter==='READY') return Math.abs(s)>=threshold;
  ...
  document.getElementById('researchGrid').innerHTML=filtered.map(r=>{{
    ...
    const isReady=Math.abs(score)>=threshold;
    const cls=isReady?'ready':score>0?'positive':score<0?'negative':'';
    const scoreCls=isReady?'ready':score>0?'pos':score<0?'neg':'zero';
    const pillCls=isReady?'ready':'wait';
    const pillText=isReady?(score>0?'READY LONG':'READY SHORT'):'WAIT';
    ...
    return `<div class="coin ${{cls}}" data-signal="${{r.ui}}">
      ...
      <form class="card-action" method="post" action="/trade/open" onsubmit="freshSubmit(event);return false;">
        <input type="hidden" name="nonce" value="">
        ...
        <div class="card-action-row">
          <select class="card-duration" name="duration"
            onchange="cardDuration['${{esc(r.symbol)}}']=this.value">
            <option value="300" ${{(cardDuration[r.symbol]||'600')==='300'?'selected':''}}>5m</option>
            <option value="600" ${{(cardDuration[r.symbol]||'600')==='600'?'selected':''}}>10m</option>
            <option value="900" ${{(cardDuration[r.symbol]||'600')==='900'?'selected':''}}>15m</option>
          </select>
          <button class="card-open ${{isReady?'ready':score>=0?'long':'short'}}" type="submit"
            ${{CARD_OPEN_DISABLED?'disabled':''}}>
            ${{r.ui==='BUY'||r.ui==='SELL'?'OPEN':'TEST'}} ${{score>=0?'LONG':'SHORT'}}
          </button>
        </div>
        <div class="snapshot-note">Locked to score ${{score>0?'+':''}}${{score.toFixed(2)}} · if the engine refreshes first, no order is sent.</div>
      </form>
    </div>`;
...
async def run():
    if MANUAL_LIVE_APPROVAL_ENABLED and "api-demo.bybit.com" not in core.BYBIT_REST_URL:
        raise RuntimeError("V0.3.5 demo guard: manual approval requires api-demo.bybit.com")
    if DEMO_LEVERAGE != 20:
        raise RuntimeError("V0.3.5 demo guard: DEMO_LEVERAGE must be 20")
    if MAX_LIVE_NOTIONAL_USDT != Decimal("50000"):
        raise RuntimeError("V0.3.5 demo guard: MAX_LIVE_NOTIONAL_USDT must be 50000")
    if MAX_OPEN_LINEAR_POSITIONS != 10:
        raise RuntimeError("V0.3.5 demo guard: MAX_OPEN_LINEAR_POSITIONS must be 10")
    ...
    log.info("trade-engine v0.3.5-demo")
