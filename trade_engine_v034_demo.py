...
class BybitTradeClient(core.BybitPrivateClient):
    def _post(self, path, body, allow_codes=None):
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "trade-engine-v0.3.4-demo",
            },
            method="POST",
        )
        try:
    ...
    def public_get(self, path, params=None):
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "trade-engine-v0.3.4-demo"})
        try:
            with urllib.request.urlopen(req, timeout=12) as response:
                data = json.loads(response.read().decode())
        except urllib.error.HTTPError as exc:
        ...
        with self.lock:
            ...
            for p in positions:
                ...
                if str(p.get("position_idx")) not in {"0", "None"}:
                    raise RuntimeError(
                        "Hedge mode is not supported in V0.3.4"
                    )
...
.card-open.short{{background:rgba(255,91,103,.15);color:#ff7b84;border:1px solid rgba(255,91,103,.38)}}
...
const CARD_NONCE='{html.escape(nonce_open)}';
...
function render(payload){{
  ...
  document.getElementById('researchGrid').innerHTML=filtered.map(r=>{{
    ...
    return `<div class="coin ${{cls}}" data-signal="${{r.ui}}">
      ...
      <form class="card-action" method="post" action="/trade/open">
        <input type="hidden" name="nonce" value="${{esc(CARD_NONCE)}}">
        ...
        <div class="card-action-row">
          ...
          <button class="card-open ${{score>=0?'long':'short'}}" type="submit"
...
async def run():
    ...
    if DEMO_LEVERAGE != 20:
        raise RuntimeError("V0.3.4 demo guard: DEMO_LEVERAGE must be 20")
    if MAX_LIVE_NOTIONAL_USDT != Decimal("50000"):
        raise RuntimeError("V0.3.4 demo guard: MAX_LIVE_NOTIONAL_USDT must be 50000")
    if MAX_OPEN_LINEAR_POSITIONS != 10:
        raise RuntimeError("V0.3.4 demo guard: MAX_OPEN_LINEAR_POSITIONS must be 10")
    ...
    log.info("trade-engine v0.3.4-demo")
