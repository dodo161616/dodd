...
VERSION = "v0.6.0-micro-2m-auto-demo"
...
MICRO_ENTRY_THRESHOLD = float(os.getenv("MICRO_ENTRY_THRESHOLD", "64"))
MICRO_EXCEPTION_THRESHOLD = float(os.getenv("MICRO_EXCEPTION_THRESHOLD", "78"))
MICRO_ENTRY_CHECK_SEC = float(os.getenv("MICRO_ENTRY_CHECK_SEC", "5"))
MICRO_CONFIRMATIONS = int(os.getenv("MICRO_CONFIRMATIONS", "3"))
MICRO_COOLDOWN_SEC = int(os.getenv("MICRO_COOLDOWN_SEC", "30"))

MICRO_MAX_SPREAD_BPS = float(os.getenv("MICRO_MAX_SPREAD_BPS", "2.5"))
MICRO_MIN_DEPTH_MULT = float(os.getenv("MICRO_MIN_DEPTH_MULT", "3.0"))
MICRO_MIN_TURNOVER_60S = float(os.getenv("MICRO_MIN_TURNOVER_60S", "150000"))
MICRO_MIN_RANGE_120_BPS = float(os.getenv("MICRO_MIN_RANGE_120_BPS", "16"))
...
class Micro2MManager(v5.AutoDemoManager):
    ...
    def candidate_ok_micro(self, m, total_open):
        ...
        if abs(m["score"]) < threshold:
            return False, None, "score"
        if not all(m["gates"].values()):
            return False, None, "quality gate"
        if not self.confirmation_ok(m, threshold):
            return False, None, "confirmation"

        closed = self.recent_close_ms(m["symbol"])
        if closed and core.now_ms() - closed < MICRO_COOLDOWN_SEC * 1000:
            return False, None, "cooldown"

        return True, slot_type, ""
    ...
    def evaluate_entries_micro(self):
        ...
        for m in snapshots:
            ...
            ok, slot_type, _ = self.candidate_ok_micro(m, total_open)
            if not ok:
                continue
...
Hard gates reject wide spread, weak 2m range versus fees, low turnover, shallow top-10 depth, V0.4 conflict and strong opposite context.
Signal must survive <b>3 distinct market snapshots</b> before entry. New trades close at <b>120 seconds</b> regardless of PnL.
</div></div>
...
function signalCard(m){{const ready=Math.abs(Number(m.score))>=Number(S.config.micro_threshold)&&Object.values(m.gates||{{}}).every(Boolean);const ex=Math.abs(Number(m.score))>=Number(S.config.exception_threshold);return `<div class="mon ${{ready?'ready':''}} ${{ex?'exception':''}}"><div class="sym">${{e(m.symbol)}}</div><div class="score ${{Number(m.score)>=0?'long':'short'}}">${{Number(m.score)>0?'+':''}}${{Number(m.score).toFixed(2)}}</div><div class="tiny">15s ${{num(m.ret15_pct)}}% · 30s ${{num(m.ret30_pct)}}% · spread ${{num(m.spread_bps)}}bp<br>flow ${{num(m.flow)}} · book ${{num(m.book)}} · micro ${{num(m.microprice)}} · OI ${{num(m.oi)}}<br>range2m ${{num(m.range120_bps)}}bp · depth $${{Number(m.depth10_notional||0).toLocaleString(undefined,{{maximumFractionDigits:0}})}} · hist N=${{m.history?.n||0}}</div></div>`}}
...
render(S);setInterval(timers,250);setInterval(poll,2000);
        ...
        def do_POST(self):
            self.send_json({"error": "V0.6 has no manual order endpoint"}, 405)
    ...
    log.info("V0.6 HTTP endpoint on 0.0.0.0:%s /health /trade /admin", core.PORT)
...
async def run():
    if AUTO_DEMO_TRADING_ENABLED and "api-demo.bybit.com" not in core.BYBIT_REST_URL:
        raise RuntimeError("V0.6 hard guard: AUTO trading is DEMO-only")
    if v4.DEMO_LEVERAGE != 20:
        raise RuntimeError("V0.6 guard: DEMO_LEVERAGE must be 20")
    if AUTO_NOTIONAL_USDT != Decimal("25000"):
        raise RuntimeError("V0.6 guard: AUTO_NOTIONAL_USDT must be 25000")
    if AUTO_HOLD_SEC != 120:
        raise RuntimeError("V0.6 guard: AUTO_HOLD_SEC must be 120")
    if AUTO_BASE_SLOTS != 6 or AUTO_EXCEPTION_SLOTS != 4:
        raise RuntimeError("V0.6 guard: slots must remain 6 + 4")
    if AUTO_TOTAL_SLOTS != 10 or v4.MAX_OPEN_LINEAR_POSITIONS != 10:
        raise RuntimeError("V0.6 guard: total open cap must remain 10")
